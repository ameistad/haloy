...
If no path is provided, the current directory is used.`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if appConfig.Hooks == nil {
				ui.Info("Debug: No hooks found in config")
			} else {
				ui.Info("Debug: Found hooks - PreDeploy: %d, PostDeploy: %d",
					len(appConfig.Hooks.PreDeploy), len(appConfig.Hooks.PostDeploy))
			}

			if appConfig.Hooks != nil && len(appConfig.Hooks.PreDeploy) > 0 {
				ui.Info("Executing pre-deploy hooks...")
				...
				ui.Success("Pre-deploy hooks completed successfully.")
			}
